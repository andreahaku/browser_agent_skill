#!/usr/bin/env bun
/**
 * browser-agent: Chrome automation CLI via DevTools Protocol (CDP)
 * Uses raw WebSocket — no Playwright dependency.
 */
...
const BASE_URL =
  process.env.BROWSER_AGENT_CDP_URL || `http://127.0.0.1:${PORT}`;
const TIMEOUT = Number(process.env.BROWSER_AGENT_TIMEOUT_MS || 10000);
...
function resolveTabIndex(pages, tabArg) {
  ...
  if (!Number.isInteger(index) || index < 0) die(`Invalid tab index: ${tabArg}`);
  if (index >= pages.length) die(`Tab ${index} out of range (0-${Math.max(0, pages.length - 1)}).`);
  return index;
}

async function connectPage(baseUrl, tabIndex, timeout = TIMEOUT) {
  const pages = await getPages(baseUrl);
  if (pages.length === 0) die("No tabs open in Chrome.");
...
function parseGlobalArgs(rawArgs) {
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    const a = rawArgs[i];
    if (a === "--cdp-url") { cdpUrl = rawArgs[++i] || die("Missing --cdp-url value."); continue; }
    if (a.startsWith("--cdp-url=")) { cdpUrl = a.slice(10); continue; }
    if (a === "--timeout-ms") { timeoutMs = Number(rawArgs[++i]); continue; }
    if (a.startsWith("--timeout-ms=")) { timeoutMs = Number(a.slice(13)); continue; }
    args.push(a);
  }
...
function takeOption(args, names) {
  for (let i = 0; i < args.length; i++) {
    for (const name of names) {
      if (args[i] === name) {
        const val = args[i + 1];
        if (!val) die(`Missing value for ${name}.`);
        args.splice(i, 2);
        return val;
      }
...
async function cmdOpen(runtime, commandArgs) {
  ...
  // --parallel: open multiple URLs in new tabs concurrently
  if (parallel) {
    const urls = args.filter((a) => !a.startsWith("-"));
    if (urls.length === 0) die("Usage: browser.js open <url1> <url2> ... --parallel");
    ...
    const results = await Promise.allSettled(
      urls.map(async (rawUrl) => {
        ...
        try {
          ...
          // Connect to the new tab and wait for load
          const pages = await getPages(runtime.cdpUrl);
          ...
          if (page) {
            ...
            try {
              ...
              return { idx: pages.indexOf(page), title: titleRes?.result?.value, url: page.url || url };
            } finally {
              client.close();
            }
          }
          return { idx: "?", title: "(loading...)", url };
        } finally {
          browser.close();
        }
      }),
  ...
  const rawUrl = args[0];
  if (!rawUrl) die("Usage: browser.js open <url> [--new-tab] [--tab <index>] [--parallel]");
  if (newTab && tabTarget !== undefined) die("Use --new-tab or --tab, not both.");
  ...
  if (newTab) {
    await withBrowser(runtime.cdpUrl, runtime.timeoutMs, async (browser) => {
      const { targetId } = await browser.send("Target.createTarget", { url });
      // Wait for target to register, then get its info
      await new Promise((r) => setTimeout(r, 500));
      const pages = await getPages(runtime.cdpUrl);
      const page = pages.find((p) => p.id === targetId);
    ...
    await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
      ...
      if (nav.errorText) die(`Navigation error: ${nav.errorText}`);
      await waitForLoad(client, runtime.timeoutMs);
...
async function cmdClick(runtime, commandArgs) {
  const args = [...commandArgs];
  if (args.length === 0) die("Usage: browser.js click [tab] <index|text>");
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    ...
    const script = isNumeric
      ? clickByIndexScript(Number(target))
      : clickByTextScript(target);
    ...
    if (!result?.ok) die(result?.reason || "Click failed.");
...
async function cmdType(runtime, commandArgs) {
  ...
  if (args.length === 0) die("Usage: browser.js type [tab] <text> [--selector <css>] [--append]");
...
async function cmdUpload(runtime, commandArgs) {
  ...
  if (args.length === 0) die("Usage: browser.js upload [tab] <file> [--selector <css>]");
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    ...
    if (!nodeId) die(`No upload input matched selector: ${selector}`);
...
async function cmdSearch(runtime, commandArgs) {
  ...
  if (!query) die("Usage: browser.js search <query>");
  ...
  for (const hit of hits) {
    const reasons = [];
    if (hit.titleMatch) reasons.push("title");
    if (hit.urlMatch) reasons.push("url");
    if (hit.snippet) reasons.push("content");

    console.log(`[${hit.index}] ${hit.title || "(no title)"}`);
    console.log(`    ${hit.url}`);
    console.log(`    match: ${reasons.join(", ")}`);
    if (hit.snippet) console.log(`    snippet: ${hit.snippet}`);
  }
}

async function cmdClose(runtime, commandArgs) {
  const target = commandArgs[0];
  if (!target) die("Usage: browser.js close <tab|all>");
...
async function cmdEval(runtime, commandArgs) {
  ...
  if (!expression) die("Usage: browser.js eval [tab] <javascript>");
...
async function cmdWait(runtime, commandArgs) {
  ...
  if (!selectorOrExpr) die("Usage: browser.js wait [tab] <css-selector|js-expression> [--timeout ms]");

  // Determine if it's a CSS selector or JS expression
  const isSelector = /^[#.\[a-zA-Z]/.test(selectorOrExpr) && !selectorOrExpr.includes("(");
  const expression = isSelector
    ? `!!document.querySelector(${JSON.stringify(selectorOrExpr)})`
    : selectorOrExpr;
...
async function cmdBack(runtime, commandArgs) {
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    ...
    if (currentIndex <= 0) die("No previous history entry.");
...
async function cmdForward(runtime, commandArgs) {
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    const { currentIndex, entries } = await client.send("Page.getNavigationHistory");
    if (currentIndex >= entries.length - 1) die("No forward history entry.");
    await client.send("Page.navigateToHistoryEntry", { entryId: entries[currentIndex + 1].id });
    await waitForLoad(client, runtime.timeoutMs);
    const urlRes = await client.send("Runtime.evaluate", {
      expression: "location.href", returnByValue: true,
    ...
    if (keyName.length === 1) {
      await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
        ...
        await client.send("Input.dispatchKeyEvent", {
          type: "keyDown", key: keyName, text: keyName,
          unmodifiedText: keyName, windowsVirtualKeyCode: keyName.charCodeAt(0),
        });
        await client.send("Input.dispatchKeyEvent", {
          type: "keyUp", key: keyName, windowsVirtualKeyCode: keyName.charCodeAt(0),
        });
...
async function cmdScroll(runtime, commandArgs) {
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    ...
    if (lower === "top") expression = "window.scrollTo(0, 0); 'Scrolled to top'";
    else if (lower === "bottom") expression = "window.scrollTo(0, document.body.scrollHeight); 'Scrolled to bottom'";
    else if (lower === "up") expression = "window.scrollBy(0, -window.innerHeight * 0.8); 'Scrolled up'";
    else if (lower === "down") expression = "window.scrollBy(0, window.innerHeight * 0.8); 'Scrolled down'";
    else if (lower === "left") expression = "window.scrollBy(-window.innerWidth * 0.8, 0); 'Scrolled left'";
    else if (lower === "right") expression = "window.scrollBy(window.innerWidth * 0.8, 0); 'Scrolled right'";
    else if (/^-?\d+$/.test(target)) expression = `window.scrollBy(0, ${target}); 'Scrolled by ${target}px'`;
    else {
...
async function cmdSelect(runtime, commandArgs) {
  const args = [...commandArgs];
  const selector = takeOption(args, ["--selector", "-s"]);
  if (args.length === 0) die("Usage: browser.js select [tab] <value> --selector \"css\"");
...
async function cmdHover(runtime, commandArgs) {
  const args = [...commandArgs];
  if (args.length === 0) die("Usage: browser.js hover [tab] <index|text|--selector css>");
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client, idx }) => {
    ...
    if (!coords) die("Element not found for hover.");

    await client.send("Input.dispatchMouseEvent", {
      type: "mouseMoved", x: coords.x, y: coords.y,
    });
    // Small delay for hover effects to trigger
...
async function cmdCookies(runtime, commandArgs) {
  ...
  if (subcommand === "get" || subcommand === "list") {
    ...
    const pair = args[1];
    if (!pair || !pair.includes("=")) die("Usage: browser.js cookies set name=value --domain example.com [--path /] [--httpOnly] [--secure]");
    ...
    await withPage(runtime.cdpUrl, "0", runtime.timeoutMs, async ({ client }) => {
      ...
      await client.send("Network.setCookie", {
        name, value, domain: cookieDomain, path: pathVal, httpOnly, secure,
      });
      console.log(`Set cookie: ${name}=${value} (domain=${cookieDomain})`);
    });
  } else if (subcommand === "delete" || subcommand === "remove") {
    const name = args[1];
    if (!name) die("Usage: browser.js cookies delete <name>");
    await withPage(runtime.cdpUrl, "0", runtime.timeoutMs, async ({ client }) => {
      await client.send("Network.enable");
      const { cookies } = await client.send("Network.getCookies");
      const matching = cookies.filter((c) => c.name === name);
      if (matching.length === 0) die(`No cookie found with name: ${name}`);
...
async function cmdStorage(runtime, commandArgs) {
  ...
  if (subcommand === "get" || subcommand === "list") {
    ...
    if (!key) die(`Usage: browser.js storage set <key> <value> [--session]`);
    ...
    const key = args[1];
    if (!key) die(`Usage: browser.js storage delete <key> [--session]`);
...
async function cmdConsole(runtime, commandArgs) {
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client }) => {
    ...
    // Intercept messages for console events
    const origWs = client;
    ...
    // We can read existing console by evaluating
    // Also listen for new messages during the duration
    ...
    // Install console interceptor for future use
    await client.send("Runtime.evaluate", {
      expression: `(() => {
...
async function cmdNetwork(runtime, commandArgs) {
  ...
  await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs + duration, async ({ client }) => {
    ...
    // Poll for network events by checking via JS
    // CDP events need special handling; we'll use a timed approach
...
async function cmdEmulate(runtime, commandArgs) {
  ...
  if (presets[presetName]) {
    ...
    await withPage(runtime.cdpUrl, tabArg, runtime.timeoutMs, async ({ client }) => {
      await client.send("Emulation.setDeviceMetricsOverride", {
        width: p.width, height: p.height, deviceScaleFactor: p.dpr, mobile: p.mobile,
      });
      if (p.ua) {
  ...
  // Custom: emulate WIDTHxHEIGHT [--dpr N] [--mobile] [--ua "string"]
...
async function cmdParallel(runtime, commandArgs) {
  // Each arg is a quoted command string: parallel "content 0" "content 1" "screenshot 2"
  const cmds = commandArgs.filter((a) => a.trim());
  if (cmds.length === 0) {
    die(
      'Usage: browser.js parallel "cmd1 [args]" "cmd2 [args]" ...\n' +
        'Example: parallel "content 0" "content 1" "screenshot 2 -o shot.png"',
    );
  }

  // Spawn each command as a separate Bun subprocess for clean stdout isolation
  const scriptPath = import.meta.filename;
  const globalArgs = [];
  if (runtime.cdpUrl !== BASE_URL) globalArgs.push("--cdp-url", runtime.cdpUrl);
  if (runtime.timeoutMs !== TIMEOUT) globalArgs.push("--timeout-ms", String(runtime.timeoutMs));

  const t0 = Date.now();
  const results = await Promise.allSettled(
    cmds.map(async (cmdStr) => {
  ...
  // Print results grouped by command
  for (const r of results) {
    if (r.status === "fulfilled") {
...
async function main() {
  ...
  switch (command) {
    case "list": return cmdList(runtime);
    case "open": return cmdOpen(runtime, args);
    case "content": return cmdContent(runtime, args);
    case "elements": return cmdElements(runtime, args);
    case "click": return cmdClick(runtime, args);
    case "type": return cmdType(runtime, args);
    case "upload": return cmdUpload(runtime, args);
    case "screenshot": return cmdScreenshot(runtime, args);
    case "html": return cmdHtml(runtime, args);
    case "search": return cmdSearch(runtime, args);
    case "close": return cmdClose(runtime, args);
    case "eval": case "js": return cmdEval(runtime, args);
    case "pdf": return cmdPdf(runtime, args);
    case "wait": return cmdWait(runtime, args);
    case "back": return cmdBack(runtime, args);
    case "forward": return cmdForward(runtime, args);
    case "reload": return cmdReload(runtime, args);
    case "keypress": case "key": return cmdKeypress(runtime, args);
    case "scroll": return cmdScroll(runtime, args);
    case "select": return cmdSelect(runtime, args);
    case "hover": return cmdHover(runtime, args);
    case "cookies": case "cookie": return cmdCookies(runtime, args);
    case "storage": return cmdStorage(runtime, args);
    case "console": return cmdConsole(runtime, args);
    case "network": case "net": return cmdNetwork(runtime, args);
    case "emulate": return cmdEmulate(runtime, args);
    case "parallel": return cmdParallel(runtime, args);
